...
> **TL;DR:** We evaluate the predictive utility of "workspace noise" (J-space entropy) against output logprobs for detecting incorrect answers in Qwen3-4B across seven distinct datasets. Across fact-retrieval tasks (TriviaQA, PopQA, NQ-Open, HotpotQA), workspace noise consistently improves routing precision at low escalation budgets by intercepting high-confidence confabulations. However, we document three specific boundary failures where the signal breaks down: (1) on TruthfulQA, the metric fails to detect errors on adversarially constructed misconceptions; (2) on GSM8K (math), the baseline entropy for correct answers shifts significantly upwards, breaking absolute thresholds; and (3) on CommonSenseQA, the multiple-choice format artificially compresses entropy, rendering the signal weaker than standard logprobs.

**Attribution:** This project builds upon the "Global Workspace in Language Models" theory and the Jacobian Lens mathematical framework introduced by Anthropic (July 6, 2026). It extends the initial open-source hypothesis by `solarkyle`, which linked workspace noise to hallucination prediction, by rigorously stress-testing the zero-shot transferability and format limitations of the metric.
...
We evaluated ~12,000 examples across seven distributions to isolate specific cognitive tasks:
...
In open-ended fact-retrieval datasets (TriviaQA, PopQA, NQ-Open, HotpotQA), filtering high-confidence predictions by workspace noise yields higher precision for identifying incorrect answers than filtering by output logprobs alone at tight routing budgets (5%-10%).
...
*Table: PopQA routing performance. At a 5% budget to route queries to a more expensive model/human review, output confidence correctly identified 87.5% hallucinations (12.5% false positive waste). Workspace noise routed with 100% precision (0 waste).*
...
*   **Format Compression (CommonSenseQA):** On CommonSenseQA, the multiple-choice format (A, B, C, D, E) artificially compressed the model's vocabulary distribution. The noise signal became significantly weaker globally than baseline output confidence (Workspace AUC 0.589 vs Confidence AUC 0.844).
...
Across all seven datasets, the separation gap between correct and incorrect answers (measured by Cohen's d) peaked maximally at the final layer (**Layer 34**). The strongest, most parsimonious, and leakage-free predictor of an epistemic failure is the absolute entropy level at the final representation layer immediately prior to output generation.
...
*   **Sequence Truncation:** Due to hardware constraints, evaluations were restricted to sequences under 512 tokens. This resulted in significant sample truncation for generative datasets (GSM8K) and long-prompt datasets (TruthfulQA), introducing a selection bias toward shorter reasoning tasks.
